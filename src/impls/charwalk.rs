...
pub fn walk<R: ResolverRules, S: WordSink>(s: &str, sink: &mut S) -> Result<(), S::Err> {
    ...
    while let Some(c) = walk.next() {
        ...
        impl_parsing_for!(
            rules.non_punct_special_rules,
            rules.non_punct_special.contains(c)
        );
        impl_parsing_for!(
            rules.case_change,
            prev_char.is_some()
        ...
        // `!a && (!b || (b && c))` is `!a && (!b || c)`: the second conjunct only reaches the
        // inner test when `b` holds, so restating it there decides nothing. The longer form
        // read as "if committing, only at the end of a bound", which is what the shorter one
        // says too.
        if !flag_to_delete && (!flag_to_commit || bound_end) {
        ...
        if is_last || (!flag_to_delete && bound_start && bound_end) {
            ...
            let already_committed = flag_to_commit && bound_end && !bound_start && !flag_to_delete;
